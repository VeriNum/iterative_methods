...
(*
Lemma extract_fst_list {A B} (v1: list A) (v2: list B) :
  let L := combine v1 v2 in
  fst (List.split L) = v1.
Proof.
simpl. rewrite combine_split /=.
*)
...
(*
Lemma fold_left_for_list {A B}  (L: list B) (p : B)
  (f: A -> B -> A) (b :  B) (a : A) : 
  In p L ->
  (forall q (a' : A) , 
              In q L -> p <> q  ->
              f a' q = a') -> 
  fold_left f L a = f a p.
Proof.
intros.
